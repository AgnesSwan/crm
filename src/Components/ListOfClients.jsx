            ...
            {clients?.map((client) =>
                <li className="w-11/12 p-2 border flex flex-row mb-4" key={client.id}>
                    ...
                    <div className="w-2/5 m-0">
                        ...
                        <div className='flex flex-row mt-2'> {client.city && client.city?.map(c => <p className="mr-2 text-sm border border-sky-500 w-14 h-6 rounded bg-gray-100 opacity-70 flex items-center justify-center">{c}</p>)}</div>
                    ...
                    <div className="w-2/5 m-0 flex items-end flex-col">
                        {client.phone && <h1 className="text-blue-500 ">{(client.phone).toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ")}</h1>}
                        {client.email && <h1 className="text-blue-500 ">{client.email}</h1>}
                    </div>
                </li>
            )}
        </ul>
