            ...
            {houses.map((house) =>
                <li className="w-11/12 p-2 mb-4 border flex flex-row" key={house.id}>
                    ...
                    <div className="w-2/5 m-0">
                        <Link to={`houseList/${house.id}`}>{house.title && <h1 className="font-semibold text-xl hover:text-blue-500">{house.title}</h1>}</Link>
                    ...
                    <div className="w-2/5 m-0 flex items-end flex-col">
                       {house.price && <h1 className="text-blue-500 text-lg">{(house.price).toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ")} PLN</h1>}
                       {(house.price && house.metric) && <h3 className="text-gray-400 text-xs">{(house.price / house.metric).toFixed(2)} PLN/m2</h3>}
                    </div>
